...
public class DlgOrdenSoporte extends JDialog implements ActionListener {
	...
	void cargarTecnicos() {
		...
		try {
			List<Tecnico> lsTecnicos = manager.createQuery(jpql, Tecnico.class).getResultList();
			
			for (Tecnico tecnico : lsTecnicos) 
	...
	void cargarClientes() {
		...
		try {
			List<Cliente> lsClientes = manager.createQuery(jpql, Cliente.class).getResultList();
